...
public class PathInterpolatorCompat implements Interpolator {
    ...
    private void initPath(Path path) {
        ...
        for (int i2 = 0; i2 < numPoints; i2++) {
            ...
            this.f21mX[i2] = position[0];
            this.f22mY[i2] = position[1];
        }
        if (((double) Math.abs(this.f21mX[0])) > 1.0E-5d || ((double) Math.abs(this.f22mY[0])) > 1.0E-5d || ((double) Math.abs(this.f21mX[numPoints - 1] - 1.0f)) > 1.0E-5d || ((double) Math.abs(this.f22mY[numPoints - 1] - 1.0f)) > 1.0E-5d) {
            throw new IllegalArgumentException("The Path must start at (0,0) and end at (1,1) start: " + this.f21mX[0] + "," + this.f22mY[0] + " end:" + this.f21mX[numPoints - 1] + "," + this.f22mY[numPoints - 1]);
        }
        float prevX = 0.0f;
        int componentIndex = 0;
        while (i < numPoints) {
            int componentIndex2 = componentIndex + 1;
            float x = this.f21mX[componentIndex];
            if (x < prevX) {
    ...
    public float getInterpolation(float t) {
        ...
        return ((this.f22mY[endIndex] - startY) * ((t - this.f21mX[startIndex]) / xRange)) + startY;
    }
}
