...
public class NavigationView extends ScrimInsetsFrameLayout {
    ...
    public NavigationView(Context context, AttributeSet attrs) {
        this(context, attrs, C0044R.attr.navigationViewStyle);
    }
    ...
    public NavigationView(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        TintTypedArray a = ThemeEnforcement.obtainTintedStyledAttributes(context, attrs, C0044R.styleable.NavigationView, defStyleAttr, C0044R.style.Widget_Design_NavigationView, new int[0]);
        ViewCompat.setBackground(this, a.getDrawable(C0044R.styleable.NavigationView_android_background));
        if (a.hasValue(C0044R.styleable.NavigationView_elevation)) {
            ViewCompat.setElevation(this, (float) a.getDimensionPixelSize(C0044R.styleable.NavigationView_elevation, 0));
        }
        ViewCompat.setFitsSystemWindows(this, a.getBoolean(C0044R.styleable.NavigationView_android_fitsSystemWindows, false));
        this.maxWidth = a.getDimensionPixelSize(C0044R.styleable.NavigationView_android_maxWidth, 0);
        if (a.hasValue(C0044R.styleable.NavigationView_itemIconTint)) {
            itemIconTint = a.getColorStateList(C0044R.styleable.NavigationView_itemIconTint);
        } else {
            itemIconTint = createDefaultColorStateList(16842808);
        }
        boolean textAppearanceSet = false;
        int textAppearance = 0;
        if (a.hasValue(C0044R.styleable.NavigationView_itemTextAppearance)) {
            textAppearance = a.getResourceId(C0044R.styleable.NavigationView_itemTextAppearance, 0);
            textAppearanceSet = true;
        }
        ColorStateList itemTextColor = a.hasValue(C0044R.styleable.NavigationView_itemTextColor) ? a.getColorStateList(C0044R.styleable.NavigationView_itemTextColor) : null;
        if (!textAppearanceSet && itemTextColor == null) {
            itemTextColor = createDefaultColorStateList(16842806);
        }
        Drawable itemBackground = a.getDrawable(C0044R.styleable.NavigationView_itemBackground);
        if (a.hasValue(C0044R.styleable.NavigationView_itemHorizontalPadding)) {
            this.presenter.setItemHorizontalPadding(a.getDimensionPixelSize(C0044R.styleable.NavigationView_itemHorizontalPadding, 0));
        }
        int itemIconPadding = a.getDimensionPixelSize(C0044R.styleable.NavigationView_itemIconPadding, 0);
        ...
        if (a.hasValue(C0044R.styleable.NavigationView_menu)) {
            inflateMenu(a.getResourceId(C0044R.styleable.NavigationView_menu, 0));
        }
        if (a.hasValue(C0044R.styleable.NavigationView_headerLayout)) {
            inflateHeaderView(a.getResourceId(C0044R.styleable.NavigationView_headerLayout, 0));
        }
        a.recycle();
    }
    ...
    private ColorStateList createDefaultColorStateList(int baseColorThemeAttr) {
        ...
        if (!getContext().getTheme().resolveAttribute(C0395R.attr.colorPrimary, value, true)) {
            return null;
        }
