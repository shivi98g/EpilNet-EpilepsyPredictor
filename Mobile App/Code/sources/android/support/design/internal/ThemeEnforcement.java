...
public final class ThemeEnforcement {
    private static final int[] APPCOMPAT_CHECK_ATTRS = {C0044R.attr.colorPrimary};
    private static final String APPCOMPAT_THEME_NAME = "Theme.AppCompat";
    private static final int[] MATERIAL_CHECK_ATTRS = {C0044R.attr.colorSecondary};
    ...
    private static void checkCompatibleTheme(Context context, AttributeSet set, @AttrRes int defStyleAttr, @StyleRes int defStyleRes) {
        TypedArray a = context.obtainStyledAttributes(set, C0044R.styleable.ThemeEnforcement, defStyleAttr, defStyleRes);
        boolean enforceMaterialTheme = a.getBoolean(C0044R.styleable.ThemeEnforcement_enforceMaterialTheme, false);
        a.recycle();
    ...
    private static void checkTextAppearance(Context context, AttributeSet set, @StyleableRes int[] attrs, @AttrRes int defStyleAttr, @StyleRes int defStyleRes, @StyleableRes int... textAppearanceResIndices) {
        TypedArray themeEnforcementAttrs = context.obtainStyledAttributes(set, C0044R.styleable.ThemeEnforcement, defStyleAttr, defStyleRes);
        boolean validTextAppearance = false;
        if (!themeEnforcementAttrs.getBoolean(C0044R.styleable.ThemeEnforcement_enforceTextAppearance, false)) {
        ...
        } else if (themeEnforcementAttrs.getResourceId(C0044R.styleable.ThemeEnforcement_android_textAppearance, -1) != -1) {
